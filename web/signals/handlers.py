...
def create_profile_for_new_user(sender, **kwargs):
    ...
    if kwargs['created']:
        if kwargs['instance'].__dict__['is_mentor']:
            ProfileMentor.objects.get_or_create(user=kwargs['instance'])
            print("-----automatically: Mentor Profile-----")
        else:
            ProfileStud.objects.get_or_create(user=kwargs['instance'])
...
def create_RequestedProjRelation_for_new_user(sender, **kwargs):
    if kwargs['created']:
        RequestedProjects.objects.get_or_create(user=kwargs['instance'])
